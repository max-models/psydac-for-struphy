...
class DirectSolver(InverseLinearOperator):
    ...
    not actually change across repeated `solve()` calls -- e.g. a time-independent
    operator solved once per time step (or once per Picard/Newton outer iteration, etc.)
    with only the right-hand side changing. A single sparse LU factorization
    (`psydac.linalg.direct_solvers.SparseSolver`) then serves every call, instead of an
    iterative method repeating (in the worst case, all the way to `maxiter`) every
    single call.
    ...
    reassignment, which a caller may do unconditionally every step regardless of whether
    the operator's *values* actually changed. This is a deliberate, cheap-by-construction
    design, not a value comparison: `A.tosparse()` is not assumed to be cheap (composed
    operators may need a nontrivial assembly), so re-deriving and comparing it on every
    call would undo most of the point of factorizing once. The caller is therefore
    responsible for knowing that `A`'s *values* are actually constant across calls; call
    `invalidate()` explicitly if `A` does change and the factorization must be rebuilt on
    the next `solve()`.
    ...
    `psydac.linalg.direct_solvers.SparseSolver` (and therefore this class) does not
    attempt.
    ...
    def solve(self, b, out=None):
        ...
        # see psydac.linalg.stencil.StencilVector.toarray_local.
        idx = tuple(
            slice(m * p, -m * p) if p != 0 else slice(0, None)
            for p, m in zip(out.pads, out.space.shifts)
        )
        out._data[idx] = x_flat.reshape(out._data[idx].shape, order='C')
